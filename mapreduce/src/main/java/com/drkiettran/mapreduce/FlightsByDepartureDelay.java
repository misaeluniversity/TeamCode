 ...
 *
 */
public class FlightsByDepartureDelay {
	public static void main(String[] args) throws Exception {
		Job job = Job.getInstance();
		job.setJarByClass(FlightsByDepartureDelay.class);
		job.setJobName("FlightsByDepartureDelay");

		TextInputFormat.addInputPath(job, new Path(args[0]));
		job.setInputFormatClass(TextInputFormat.class);

		job.setMapperClass(FlightsByDepartureDelayMapper.class);
		job.setReducerClass(FlightsByDepartureDelayReducer.class);
