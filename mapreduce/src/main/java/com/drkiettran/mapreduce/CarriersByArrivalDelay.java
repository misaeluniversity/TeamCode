 ...
 *
 */
public class CarriersByArrivalDelay {
	public static void main(String[] args) throws Exception {
		Job job = Job.getInstance();
		job.setJarByClass(CarriersByArrivalDelay.class);
		job.setJobName("CarriersByArrivalDelay");

		TextInputFormat.addInputPath(job, new Path(args[0]));
		job.setInputFormatClass(TextInputFormat.class);

		job.setMapperClass(CarriersByArrivalDelayMapper.class);
		job.setReducerClass(CarriersByArrivalDelayReducer.class);
