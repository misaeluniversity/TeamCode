...
public class FlightsAvgDepartureDelay {
	public static void main(String[] args) throws Exception {
		Job job = Job.getInstance();
		job.setJarByClass(FlightsAvgDepartureDelay.class);
		job.setJobName("FlightsAvgDepartureDelay");

		TextInputFormat.addInputPath(job, new Path(args[0]));
		job.setInputFormatClass(TextInputFormat.class);

		job.setMapperClass(FlightsAvgDepartureDelayMapper.class);
		job.setReducerClass(FlightsAvgDepartureDelayReducer.class);
		...
		private static void resultToMongo(String resultFile) throws IOException {
			...
			while (line != null) {
				...
				while (cursor.hasNext()) {
					if(cursor.next().containsField("avgDeptDelay"))
						exist = true;				
				}
				if(exist) {
					BasicDBObject data = new BasicDBObject();
					data.put("delay",delay);
					collection.update(searchQ, new BasicDBObject("$push",new BasicDBObject("avgDeptDelay",data)));
				}else {
					java.util.List<BasicDBObject> datadelay = new ArrayList<>();
					BasicDBObject data = new BasicDBObject();
					data.put("delay", delay);
					datadelay.add(data);
					collection.update(searchQ, new BasicDBObject("$set",new BasicDBObject("avgDeptDelay",datadelay)));
				}
