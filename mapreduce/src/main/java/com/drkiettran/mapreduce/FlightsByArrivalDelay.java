 ...
 *
 */
public class FlightsByArrivalDelay {
	public static void main(String[] args) throws Exception {
		Job job = Job.getInstance();
		job.setJarByClass(FlightsByArrivalDelay.class);
		job.setJobName("FlightsByArrivalDelay");

		TextInputFormat.addInputPath(job, new Path(args[0]));
		job.setInputFormatClass(TextInputFormat.class);

		job.setMapperClass(FlightsByArrivalDelayMapper.class);
		job.setReducerClass(FlightsByArrivalDelayReducer.class);
